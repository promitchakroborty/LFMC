...
import model_defs_rast as model_defs
...
lf_models = [model_defs.lf3eval, model_defs.lf4eval]
surr_build = 'sel'
sel_strat = 'S'
lotp_for_pf = True
...
plt.savefig('Failure surface LFSS.pdf')
plt.close()
...
plt.savefig('Training Data LFSS.pdf')
plt.close()
...
for idx_model in range(nmodels):
    ...
    plt.ylabel('Dimension 2')
    plt.legend()
    plt.savefig('Model ' + str(idx_model+1) + ' train LFSS.pdf')
    plt.close()
...
plt.ylabel('Dimension 2')
plt.legend()
plt.savefig('Selected Model LFSS.pdf')
plt.close()
...
plt.legend()
plt.savefig('HF vs Surrogate LFSS.pdf')
plt.close()
...
plt.savefig('Cumulative HF LFSS.pdf')
plt.close()
...
plt.xlabel('Sample Index')
plt.ylabel('Surrogate response')
plt.savefig('Surrogate LFSS.pdf')
plt.close()
...
plt.legend()
plt.savefig('Model probabilities LFSS.pdf')
plt.close()
...
plt.legend()
plt.savefig('Cumulative LF calls LFSS.pdf')
plt.close()
...
for idx_model in range(nmodels):
    ...
    plt.savefig('Noise Variance Evolution for Model ' + str(idx_model) + ' LFSS.pdf')
    plt.close()
...
plt.legend()
plt.savefig('Evolution of AIC Probabilities LFSS.pdf')
plt.close()
...
for idx_model in range(nmodels):
    ...
    plt.xlabel('Training Sample Index')
    plt.ylabel('AICc')
    plt.savefig('AICc Evolution for Model ' + str(idx_model) + ' LFSS.pdf')
    plt.close()
